...
import com.bjd.smartanalysis.service.data.DataGaCrjzjService;
...
@Api(value = "出入境证件", tags = {"出入境证件"})
@RequestMapping("crjzj")
@RestController
public class DataGaCrjzjController {
    ...
    @Autowired
    private DataGaCrjzjService service;

    private DataBaseController<DataGaCrjzj> controller;
    ...
    private ResponseData UploadData(@RequestParam("file") MultipartFile file, Integer bid, Integer projectId) {
        ...
        return controller.UploadData(file, bid, DataGaCrjzj.class);
    }
    ...
    public void ExportExcel(HttpServletResponse response, Integer projectId) {
        controller = new DataBaseController<>(service, dataTypeService, fileService, basePath, projectId);
        controller.ExportExcel("出入境证件", DataGaCrjzj.class, response);
    }
    ...
    public ResponseData UpdateData(@RequestBody DataGaCrjzj body, Integer projectId){
        controller = new DataBaseController<>(service, dataTypeService, fileService, basePath, projectId);
    ...
    private ResponseData RemoveAllData(Integer projectId) {
        QueryWrapper<DataGaCrjzj> queryWrapper = new QueryWrapper<>();
