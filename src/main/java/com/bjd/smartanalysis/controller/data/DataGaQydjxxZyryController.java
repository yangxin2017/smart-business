...
import com.bjd.smartanalysis.service.data.DataGaQydjxxZyryService;
...
@Api(value = "企业登记信息-主要人员", tags = {"企业登记信息-主要人员"})
@RequestMapping("qydjxxZyry")
@RestController
public class DataGaQydjxxZyryController {
    ...
    @Autowired
    private DataGaQydjxxZyryService service;

    private DataBaseController<DataGaQydjxxZyry> controller;
    ...
    private ResponseData UploadData(@RequestParam("file") MultipartFile file, Integer bid, Integer projectId) {
        ...
        return controller.UploadData(file, bid, DataGaQydjxxZyry.class);
    }
    ...
    public ResponseData UpdateData(@RequestBody DataGaQydjxxZyry body, Integer projectId){
        controller = new DataBaseController<>(service, dataTypeService, fileService, basePath, projectId);
    ...
    public void ExportExcel(HttpServletResponse response, Integer projectId) {
        controller = new DataBaseController<>(service, dataTypeService, fileService, basePath, projectId);
        controller.ExportExcel("企业登记信息-主要人员", DataGaQydjxxZyry.class, response);
    }

    @GetMapping("remove")
    @ApiOperation(value = "删除所有数据", notes = "删除所有数据")
    private ResponseData RemoveAllData(Integer projectId) {
        QueryWrapper<DataGaQydjxxZyry> queryWrapper = new QueryWrapper<>();
