...
@Api(value = "中国人民银行银行账户", tags = {"中国人民银行银行账户"})
@RequestMapping("rmyhzh")
@RestController
public class DataGaRmyhzhController {
    ...
    @Autowired
    private DataGaRmyhzhService service;

    private DataBaseController<DataGaRmyhzh> controller;
    ...
    private ResponseData UploadData(@RequestParam("file") MultipartFile file, Integer bid, Integer projectId) {
        ...
        return controller.UploadData(file, bid, DataGaRmyhzh.class);
    }
    ...
    public ResponseData UpdateData(@RequestBody DataGaRmyhzh body, Integer projectId){
        controller = new DataBaseController<>(service, dataTypeService, fileService, basePath, projectId);
    ...
    public void ExportExcel(HttpServletResponse response, Integer projectId) {
        controller = new DataBaseController<>(service, dataTypeService, fileService, basePath, projectId);
        controller.ExportExcel("中国人民银行银行账户", DataGaRmyhzh.class, response);
    }
