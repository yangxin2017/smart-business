...
import com.bjd.smartanalysis.service.data.DataGaCrjjlService;
...
@Api(value = "出入境记录", tags = {"出入境记录"})
@RequestMapping("crjjl")
@RestController
public class DataGaCrjjlController {
    ...
    @Autowired
    private DataGaCrjjlService service;

    private DataBaseController<DataGaCrjjl> controller;
    ...
    private ResponseData UploadData(@RequestParam("file") MultipartFile file, Integer bid, Integer projectId) {
        ...
        return controller.UploadData(file, bid, DataGaCrjjl.class);
    }
    ...
    public void ExportExcel(HttpServletResponse response, Integer projectId) {
        controller = new DataBaseController<>(service, dataTypeService, fileService, basePath, projectId);
        controller.ExportExcel("出入境记录", DataGaCrjjl.class, response);
    }
    ...
    public ResponseData UpdateData(@RequestBody DataGaCrjjl body, Integer projectId){
        controller = new DataBaseController<>(service, dataTypeService, fileService, basePath, projectId);
    ...
    private ResponseData RemoveAllData(Integer projectId) {
        QueryWrapper<DataGaCrjjl> queryWrapper = new QueryWrapper<>();
