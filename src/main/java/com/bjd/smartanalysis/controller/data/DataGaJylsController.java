...
import com.bjd.smartanalysis.service.data.DataGaJylsService;
...
@Api(value = "金融机构交易流水", tags = {"金融机构交易流水"})
@RequestMapping("jyls")
@RestController
public class DataGaJylsController {
    ...
    @Autowired
    private DataGaJylsService service;

    private DataBaseController<DataGaJyls> controller;
    ...
    private ResponseData UploadData(@RequestParam("file") MultipartFile file, Integer bid, Integer projectId) {
        ...
        return controller.UploadData(file, bid, DataGaJyls.class);
    }
    ...
    public ResponseData UpdateData(@RequestBody DataGaJyls body, Integer projectId){
        controller = new DataBaseController<>(service, dataTypeService, fileService, basePath, projectId);
    ...
    public void ExportExcel(HttpServletResponse response, Integer projectId) {
        controller = new DataBaseController<>(service, dataTypeService, fileService, basePath, projectId);
        controller.ExportExcel("人员电子档案", DataGaJyls.class, response);
    }
}
